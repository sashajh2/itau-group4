...
# Project root is two levels up from this file (models/time_series/ → project root)
...
def train_model(
    ...
    config: ModelConfig,
    device: torch.device,
    save_dir: str = os.path.join(PROJECT_ROOT, "checkpoints"),
):
    """
...
def main():
    ...
    # Dataset paths
    hdf5_path = os.path.join(PROJECT_ROOT, "exports", "deepfake_embeddings.h5")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        # Test mode: python -m models.time_series.time_series_model test <hdf5_path> <checkpoint_path> [filter_dataset]
        if len(sys.argv) < 4:
            print("Usage: python -m models.time_series.time_series_model test <hdf5_path> <checkpoint_path> [filter_dataset]")
            print("\nExample:")
            print("  python -m models.time_series.time_series_model test sora2_embeddings.h5 checkpoints/best_model.pt sora2")
            sys.exit(1)
