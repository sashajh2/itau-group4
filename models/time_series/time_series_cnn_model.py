...
def train_model(model: nn.Module, train_loader: DataLoader, val_loader: Optional[DataLoader], config: CNNModelConfig,
                device: torch.device, save_dir: str = os.path.join(PROJECT_ROOT, "checkpoints")) -> Dict:
...
def main():
    ...
    train_model(model, train_loader, val_loader, config, device, save_dir=os.path.join(PROJECT_ROOT, "checkpoints"))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        if len(sys.argv) < 4:
            print("Usage: python -m models.time_series.time_series_cnn_model test <hdf5_path> <checkpoint_path> [filter_dataset]")
            sys.exit(1)
